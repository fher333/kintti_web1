...
export const listProducts = ({ seller = '' }) => async (dispatch) => {
    ...
    try {
      const { data } = await Axios.get(`/api/products?seller=${seller}`); //trae de la api los datos al frontend
        dispatch({ type: PRODUCT_LIST_SUCCESS, payload: data }) //solo data
    } catch (error) {
...
export const detailsProduct = (productId) => async (dispatch) => {
    ...
    try {
        ...
        dispatch({ type: PRODUCT_DETAILS_SUCCESS, payload: data }); //solo data
    } catch (error) {
        dispatch({
